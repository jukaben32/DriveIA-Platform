import { buildDealerRealtimeContext, buildRealtimeSessionPayload } from '@/ai/tools'
...
export async function resolveRealtimeDealerContext(
  supabase: DbClient,
  input: {
    businessSlug: string
  ...
  const context = await buildDealerRealtimeContext(supabase, business.id)
