...
export async function createCheckoutSession(opts: {
  ...
  const priceId = await findOrCreatePrice(stripe, `plan_${opts.plan}`, `${planConfig.name} Plan`, planConfig.priceUsd)
...
export async function syncSubscriptionFromStripeEvent(supabase: DbClient, event: Stripe.Event): Promise<void> {
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      if (session.metadata?.addon === 'website_builder') {
        await supabase.from('business_subscriptions').update({ website_builder_enabled: true }).eq('business_id', businessId)
        return
      }
