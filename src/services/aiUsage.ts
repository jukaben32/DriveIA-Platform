...
export async function getAiSpendUsd(supabase: DbClient, businessId: string, sinceIso?: string) {
  ...
  return (data ?? []).reduce((sum, row: { cost_usd: number | null }) => sum + (row.cost_usd ?? 0), 0)
}
